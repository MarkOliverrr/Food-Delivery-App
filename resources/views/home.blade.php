...
@section('title', 'Home || Online Food Ordering System')

@section('content')
<section class="hero bg-image" data-image-src="{{ asset('images/img/pimg.jpg') }}">
            ...
            @foreach($popularDishes as $dish)
                <div class="col-xs-12 col-sm-6 col-md-4 food-item">
                    <div class="food-item-wrap">
                        <div class="figure-wrap bg-image" data-image-src="{{ asset('admin/Res_img/dishes/' . $dish->img) }}"></div>
                        <div class="content">
                            <h5><a href="{{ route('dishes.show', $dish->restaurant->rs_id) }}">{{ $dish->title }}</a></h5>
                            <div class="product-name">{{ $dish->slogan }}</div>
                            <div class="price-btn-block">
                                <span class="price">₱{{ number_format($dish->price, 2) }}</span>
                                <a href="{{ route('dishes.show', $dish->restaurant->rs_id) }}" class="btn theme-btn-dash pull-right">Order Now</a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <nav class="primary pull-left">
                        <ul>
                            ...
                            @foreach($categories as $category)
                                <li><a href="#" data-filter=".{{ $category->c_name }}">{{ $category->c_name }}</a></li>
                            @endforeach
                        </ul>
                    </nav>
                </div>
        ...
        <div class="row">
            <div class="restaurant-listing">
                @foreach($restaurants as $restaurant)
                    <div class="col-xs-12 col-sm-12 col-md-6 single-restaurant all {{ $restaurant->category->c_name ?? '' }}">
                        <div class="restaurant-wrap">
                            <div class="row">
                                <div class="col-xs-12 col-sm-3 col-md-12 col-lg-3 text-xs-center">
                                    <a class="restaurant-logo" href="{{ route('dishes.show', $restaurant->rs_id) }}">
